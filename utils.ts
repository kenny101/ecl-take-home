...
export async function readFileLineByLine(filePath: string): Promise<Output[]> {
  ...
  for await (const chunk of stream) {
    ...
    while (lines.length > 1) {
      ...
      const { score, value } = extractScoreAndValue(line);
      output.push({
        score: score,
        id: value.id,
      });
 ...
 */
export function sortOutputsByScoreDescending(
  outputs: Output[],
  n: number
): Output[] {
  const heap: Output[] = [];
...
function heapifyUp(heap: Output[], index: number) {
  ...
  while (currentIndex > 0) {
    ...
    if (
      parseFloat(heap[currentIndex].score) < parseFloat(heap[parentIndex].score)
    ) {
...
function heapifyDown(heap: Output[], index: number) {
  ...
  while (true) {
    ...
    if (
      ...
      parseFloat(heap[leftChildIndex].score) < parseFloat(heap[swapIndex].score)
    ) {
    ...
    if (
      ...
      parseFloat(heap[rightChildIndex].score) <
        parseFloat(heap[swapIndex].score)
    ) {
